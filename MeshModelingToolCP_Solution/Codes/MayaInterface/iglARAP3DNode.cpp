...
const MTypeId MiglARAP3DNode::id = 0x01100000;
const MString MiglARAP3DNode::nodeName = "iglARAP3DNode";

MObject MiglARAP3DNode::aNumIter;
MObject MiglARAP3DNode::aMaxDisplacementVisualization;
MObject MiglARAP3DNode::aDeformationWeight;
MObject MiglARAP3DNode::aHandlePositions;
MObject MiglARAP3DNode::aHandleIndices;

void* MiglARAP3DNode::creator()
{
    return new MiglARAP3DNode;
}

MStatus MiglARAP3DNode::initialize()
{
    ...
    //status = MGlobal::executeCommand("makePaintable -attrType multiFloat -sm deformer " + MiglARAP3DNode::nodeName + " weights;");
    //CHECK_MSTATUS_AND_RETURN_IT(status);
...
void MiglARAP3DNode::postConstructor()
{
...
MStatus MiglARAP3DNode::deform(MDataBlock& block, MItGeometry& iter, const MMatrix& mat, unsigned int multiIndex)
{
    ...
    if (inputChangedFlag > InputChangedFlag::Visualization)
    {
        ...
        if ((inputChangedFlag & InputChangedFlag::InputMesh) != InputChangedFlag::None)
        {
            ...
            m_operationShPtr.reset(new AAShapeUp::iglARAP3DOperation(m_geometrySolverShPtr)); 
            usingTetCache = false;
        }
